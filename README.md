...
## Lancer avec Docker (Nginx + PHP-FPM)

1. Lancez la stack:
   ```bash
   docker compose -f compose.dev.yaml up --build
   ```
2. Ouvrez le site sur `http://localhost:8000`
3. La base MariaDB est initialisée automatiquement avec `sql/import.sql` au premier démarrage.
